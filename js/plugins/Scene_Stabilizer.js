/*:
@author Chaucer
@plugindesc | Scene Stabilizer : Version - 7.4.1 | Allows asyncronous loading of assets.
@help
===============================================================================
  Introduction :
===============================================================================
...
● Date : 02/04/2020
  ...
  ● Date : 05/26/2020
    ...
    ★ Add - compatability with Audio Streaming plugin.
    ★ Add - compatability with YEP Hit Damage Sounds plugin.
    ★ Add - compatability with MOG_MenuBackground plugin.

* ● Version : 7.3.1
* ● Date : 20/09/2024
*   ✩ Fix - Error when trying to destroy an already destroyed texture.

* ● Version : 7.3.2
* ● Date : 21/12/2024
*   ✩ Fix - ignored textures sometimes still get deleted.

* ● Version : 7.4.0
* ● Date : 22/12/2024
*   ★ Add - reserveBitmap is now used to preoad instead of loadBitmap.

* ● Version : 7.4.1
* ● Date : 26/12/2024
*   ✩ Fix - fixed a random crash in some instances.
...
//-----------------------------------------------------------------------------
  if ( Chaucer.sceneStabilizer.audioStreaming() ) return true;
...
//-----------------------------------------------------------------------------
  ...
  var bitmap = ImageManager.reserveBitmap( folder, filename );
...
//-----------------------------------------------------------------------------
    ...
    if ( item && item.bitmap ) {
      ...
      if ( Chaucer.sceneStabilizer.ignoreClear.includes( key ) ) return;
      delete ImageManager._imageCache._items[key];
      if ( !item.bitmap ) return;
      if ( item.bitmap.__baseTexture && item.bitmap.__baseTexture._destroyed ) {
...
//-----------------------------------------------------------------------------
  ...
  //-----------------------------------------------------------------------------
    ...
    warning = "Failed to load ImageCache, using WebStorage cache instead!";
...
//-----------------------------------------------------------------------------
  ...
  { // return if audio streaming plugin is enabled.
//-----------------------------------------------------------------------------

    return $plugins.some( function( plugin ) {
      return plugin.name == "AudioStreaming" && plugin.status == true;
    } );
...
if ( Imported.MOG_MenuBackground ) {
  ...
  //-----------------------------------------------------------------------------
  $.alias.S_MB = Scene_MenuBase.prototype.start;
  //-----------------------------------------------------------------------------
  ...
  { // Alias of commandEnabled
    ...
    this._windowLayer.children.forEach(( child ) => {
      child.updateBackgroundOpacity();
    });

    $.alias.S_MB.call( this );
